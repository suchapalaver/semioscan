...
- Added range-first chunked log scan APIs:
  `EventScanner::scan_range`, `EventScanner::scan_with_handler_range`, and
  `fetch_logs_chunked_range`. These accept finite `RangeBounds<u64>` values
  while preserving the existing start/end APIs and rejecting open-ended or
  empty ranges. Closes #1.
...
- Fixed feature-gated rustdoc links so no-default-feature documentation builds
  do not warn on links to optional modules.
...
- A provider configuration that ends up with both a requests-per-second
  rate limit and a minimum-delay gap on the same endpoint is now rejected
  at construction time instead of silently dropping the minimum delay.
